...
void loop()
{
  ...
  while (true)
  {
    if (receivedAck)
    {
      ...
      noteActivity();
      DW1000.getData(data, LEN_DATA);
      //Serial.print("Sender: ");Serial.print(data[16]); Serial.print("      "); Serial.print(data[17]);Serial.print("     "); Serial.println(data[0]);
      if (data[16] == 3 && data[17] == 21 && data[0] == RANGE_REPORT)
      {

        start = true;
      }
    }
    else if (millis() - lastActivity > 3000) //check this reset timing
    {
      receiver();
      noteActivity();
    }
  ...
  for (i = 1; i <= noOfAnchors; i++)
  {
    ...
    //Serial.print("Distance from anchor "); Serial.print(i); Serial.print(" : "); Serial.println(distance);
  }
  //sendDistances();
