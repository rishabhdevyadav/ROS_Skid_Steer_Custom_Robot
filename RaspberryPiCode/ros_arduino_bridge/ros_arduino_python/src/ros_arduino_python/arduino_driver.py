...
class Arduino:
    ...
    def __init__(self, port="/dev/ttyACM0", baudrate=57600, timeout=0.5, motors_reversed=False):
        # Corresponds to the PID_RATE defined in ROSArduinoBridge.ino [:113]
        self.PID_RATE = 60
    ...
    def execute_vi(self, cmd): # only for voltage and current
        ''' Thread safe execution of "cmd" on the Arduino returning a single integer value.
        '''
        self.mutex.acquire()
    ...
    def get_voltage(self):
        ...
        '''
        try:
            return float(self.execute_vi('v'));
        except:
            return None
    ...
    def get_current(self):
        ...
        '''
        try:
            return float(self.execute_vi('i'));
        except:
            return None
    ...
    def get_heading(self):
        ...
        '''
        try:
            return float(self.execute_vi('h'));
        except:
            return None

    def controlPiston(self, ctrl):
        ''' Move the Piston UP, Down or Stop
            1 - UP
            2 - Down
            else - STOP
        '''
        return self.execute_ack('l %d' %(ctrl))
    
    def get_encoder_counts(self):
        #print('asjhdb')
        values = self.execute_array('e')
        #print('hulalalalalala', values)
        if len(values) != 4:
    ...
    def drive(self, AM1=0, BM1=0, AM2=0, BM2=0):
        ''' Speeds are given in encoder ticks per PID interval
        '''
        # print("In drive: ", AM1, BM1, AM2, BM2)
        if self.motors_reversed:
            AM1, BM1, AM2, BM2 = -AM1, -BM1, -AM2, -BM2
        return self.execute_ack('m %d %d %d %d' %(AM1, BM1, AM2, BM2))
