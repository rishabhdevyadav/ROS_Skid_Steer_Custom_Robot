...
class DECAWAVE():
	def __init__(self):
		...
		while(i < 50):	
			try:
				...
				i = i+1 
				rospy.logerr(e)
				rospy.loginfo("Serial Port [{}] not Found".format(self.port))
				time.sleep(1)
	...
	def update(self):
		try:
			serial_ip = self.ser.readline()
                        #print(serial_ip)
			values = map(float, serial_ip.split())
			#print(values)
			...
			print("distances recieved is not a float value, got {}".format(serial_ip))
