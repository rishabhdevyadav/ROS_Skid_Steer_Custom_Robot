...
class Formation():
	def __init__(self):
		...
		rospy.Subscriber('/'+self.LeaderName+'/f_odom' , Odometry, self.leaderOdomCallback)
		
		self.initPose()
		self.leaderOdomCallback(rospy.wait_for_message('/'+self.LeaderName+'/f_odom', Odometry))
		self.currOdomCallback(rospy.wait_for_message('/R3/f_odom', Odometry))
		...
		self.L_ijd = rospy.get_param('~L_ijd',1)#rospy.get_param('~rho',0.5)  
	...
	def update(self):
		if self.init == 1:
			...
			print(self.Y_je)
			print(self.theta_je)
			pub=rospy.Publisher('fback',String,queue_size=10)
			pub.publish(self.robotId)
