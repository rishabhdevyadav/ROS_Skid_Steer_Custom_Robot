...
class BaseController:
    def __init__(self, arduino, base_frame, name="base_controllers"):
        ...
        self.frame_id = rospy.get_param("~frame_id", "r3_odom_wheel")
        ...
        # rospy.Subscriber("piston", Int8, self.pistonCallback)
    ...
    def poll(self):
        now = rospy.Time.now()
        # self.headingPub.publish(self.arduino.get_heading())
        ...
        if now > self.t_next:
            # Read the encoders
            try:
                        # First element is used to decide whether to move it up or down
            ...
            if abs(left_enc - self.enc_left) > self.count_difference:
                left_enc = self.enc_left
                # print "taking prev count left"
            if abs(right_enc - self.enc_right) > self.count_difference:
                right_enc = self.enc_right
                # print "taking prev count right"
    ...
    # def pistonCallback(self, msg):
    #     # 1 - UP
    #     # 2 - DOWN
    #     # else - STOP
    #     self.arduino.controlPiston(msg.data)
    ...
    def cmdVelCallback(self, req):
        ...
        self.count_difference = 10000 * abs(x)
