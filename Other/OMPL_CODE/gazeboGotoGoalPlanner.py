...
def update_pose(msg):
    global poseState
    idx = msg.name.index('minion_'+str(newRobotId))
    pose = msg.pose[idx]
    theta = tf.transformations.euler_from_quaternion(
                                    [pose.orientation.__getattribute__(i) 
                                    for i in pose.orientation.__slots__])
    poseState = [pose.position.x, pose.position.y, theta[2]]
...
rospy.Subscriber('/gazebo/model_states', ModelStates, update_pose)
# rospy.Subscriber('/R'+str(newRobotId)+ '/d_odom', Odometry, update_pose)
cmd_vel_pub = rospy.Publisher('/minion_' + str(newRobotId) + \
