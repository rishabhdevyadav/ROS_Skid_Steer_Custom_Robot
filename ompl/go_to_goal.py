...
pose_path = "/home/subhasis/Desktop/temp"
import math
import numpy as np
...
if __name__ == "__main__":
    ...
    # dilation points wrt to the robot position, using in generating obstacle map
    # def_points = [[0.4, 0.25 ],\
    #               [0.4, -0.25],\
    #               [-0.2,-0.25],\
    #               [-0.2,0.25 ]]
    def_points = [[0.35, 0.25 ],\
                  [0.35, -0.25],\
                  [-0.2,-0.25],\
                  [-0.2,0.25 ]]

    # read the map file generated from the generate_map.py file
    ...
    success, path = plan(4, 'InformedRRTstar', 'PathLength', None, (0.0, bound_limit),\
                        (poseX[-2],poseY[-2]), (poseX[-1],poseY[-1]))
    if not success:
        print "Sorry, No path found ... Exiting"
        exit()
    ...
    print "Ros initializations Complete"
    # ROS initializations
